...
alloc_transaction:
	if (!journal->j_running_transaction) {
		new_transaction = hfsplus_jbd_kmalloc(sizeof(*new_transaction),
						GFP_NOFS);
		if (!new_transaction) {
			ret = -ENOMEM;
			goto out;
...
repeat:
	...
	if (buffer_dirty(bh)) {
		 ...
		 */
		HFSPLUS_JBUFFER_TRACE(jh, "Unexpected dirty buffer");
	...
	if (jh->b_transaction && jh->b_transaction != transaction) {
		...
		if (jh->b_jlist != HFSPLUS_BJ_Forget || force_copy) {
			...
			if (!frozen_buffer) {
				...
				hfsplus_jbd_unlock_bh_state(bh);
				frozen_buffer = hfsplus_jbd_kmalloc(hfsplus_jh2bh(jh)->b_size,
							    GFP_NOFS);
				if (!frozen_buffer) {
					printk(KERN_EMERG
...
/**
 * int hfsplus_jbd_get_undo_access() -  Notify intent to modify metadata with
...
repeat:
	if (!jh->b_committed_data) {
		committed_data = hfsplus_jbd_kmalloc(hfsplus_jh2bh(jh)->b_size, GFP_NOFS);
		if (!committed_data) {
...
/** 
 * int hfsplus_jbd_dirty_data() -  mark a buffer as containing dirty data which
...
int hfsplus_jbd_dirty_data(hfsplus_jbd_handle_t *handle, struct buffer_head *bh)
{
	...
	if (jh->b_transaction) {
		...
		if (jh->b_transaction != handle->h_transaction) {
			...
			/* hfsplus_jbd_clean_data_list() may have got there first */
			if (jh->b_transaction != NULL) {
...
int hfsplus_jbd_dirty_metadata(hfsplus_jbd_handle_t *handle, struct buffer_head *bh)
{
	...
	if (jh->b_transaction != transaction) {
		...
		hfsplus_jbd_debug(5, "already on other transaction\n");
		HFSPLUS_J_ASSERT_JH(jh, jh->b_transaction ==
					journal->j_committing_transaction);
	...
	HFSPLUS_J_ASSERT_JH(jh, jh->b_frozen_data == 0);

	HFSPLUS_JBUFFER_TRACE(jh, "file as HFSPLUS_BJ_Metadata");
	hfsplus_jbd_debug(5, "file as HFSPLUS_BJ_Metadata\n");
	spin_lock(&journal->j_list_lock);
 ...
 */
void __hfsplus_jbd_temp_unlink_buffer(struct hfsplus_jbd_head *jh)
{
	...
	if (jh->b_jlist != HFSPLUS_BJ_None)
		HFSPLUS_J_ASSERT_JH(jh, transaction != 0);
...
/** 
 * int hfsplus_jbd_invalidatepage() 
 ...
 */
int hfsplus_jbd_invalidatepage(hfsplus_jbd_t *journal, 
		      struct page *page, 
			...
			hfsplus_transaction_t *transaction, int jlist)
{
	 ...
	 * with __hfsplus_jbd_unexpected_dirty_buffer()'s handling of dirty
	 * state. */
...
void __hfsplus_jbd_refile_buffer(struct hfsplus_jbd_head *jh)
{
	...
	jh->b_next_transaction = NULL;
	__hfsplus_jbd_file_buffer(jh, jh->b_transaction, HFSPLUS_BJ_Metadata);

...
image_new(int32_t width, int32_t height)
{
	...
	{
		DPRINTF(E_WARN, L_METADATA, "0.malloc failed\n");
		return NULL;
	}
	...
	{
		DPRINTF(E_WARN, L_METADATA, "1.malloc failed\n");
		free(vimage);
		return NULL;
	}
	return(vimage);
...
image_new_from_jpeg(const char * path, int is_file, const char * buf, int size, int scale)
{
	...
	DPRINTF(E_WARN, L_METADATA, "%s: scale:%d\n", __FUNCTION__, scale);
	...
	if(cinfo.output_components == 3)
	{
		...
		{
			DPRINTF(E_WARN, L_METADATA, "3.malloc failed\n");
			return NULL;
		}

		...
		switch( ctx->streams[video_stream]->codec->codec_id )
		{
			...
			case CODEC_ID_H264:
						...
						switch( audio_profile )
						{
							...
							case PROFILE_AUDIO_AC3:
								asprintf(&m.dlna_pn, "AVC_TS_MP_HD_AC3%s;%s",
									ts_timestamp==NONE?"_ISO" : ts_timestamp==VALID?"_T":"", dlna_no_conv);
								break;
							/* Marked by Michael Jiang, for DLNA CTT 1.56, 2010/08/02, 
 							 * no support for AVC_TS_MP_HD_AAC_MULT5.
 							 */
				...
				else if( strcmp(ctx->iformat->name, "mov,mp4,m4a,3gp,3g2,mj2") == 0 )
				{
					    ...
					    !ends_with(path, ".mov") )
					{
						...
						{
							/* Marked by Michael Jiang, for DLNA CTT 1.56, 2010/08/02, 
 							 * no support for AVC_MP4_MP_SD_AC3 & AVC_MP4_MP_SD_AAC_MULT5 .
 							 */
							//case PROFILE_AUDIO_AC3:
							...
							//	break;
							/* Ended by Michael Jiang, for DLNA CTT 1.56, 2010/08/02. */
							default:
			...
			case CODEC_ID_MPEG4:
				...
				else if( ends_with(path, ".3gp") && (strcmp(ctx->iformat->name, "mov,mp4,m4a,3gp,3g2,mj2") == 0) )
				{
					...
					{
						/* Marked by Michael Jiang, for DLNA CTT 1.56, 2010/08/02, 
 						 * no support for MPEG4_P2_3GPP_SP_L0B_AAC & MPEG4_P2_3GPP_SP_L0B_AMR.
 						 */
			...
			case CODEC_ID_VC1:
				    ...
				    (ctx->bit_rate/8 <= 384000) )
				{
					switch( audio_profile )
					{
						/* Marked by Michael Jiang, for DLNA CTT 1.56, 2010/08/02, no support for WMVSPML_MP3 */
						//case PROFILE_AUDIO_MP3:
				         ...
				         (ctx->bit_rate/8 <= 10000000) )
				{
					switch( audio_profile )
					{
						/* Marked by Michael Jiang, for DLNA CTT 1.56, 2010/08/02, no support for WMVMED_PRO */
				         ...
				         (ctx->bit_rate/8 <= 20000000) )
				{
					switch( audio_profile )
					{
						/* Marked by Michael Jiang, for DLNA CTT 1.56, 2010/08/02, no support for WMVHIGH_PRO */
						//case PROFILE_AUDIO_WMA_PRO:
